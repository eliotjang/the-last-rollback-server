...
const monsterMoveHandler = async ({ socket, accountId, packet }) => {
  ...
  const curSession = getBattleSessionByUserId(accountId);

  if (curSession) {
    throw new CustomError(ErrorCodes.GAME_NOT_FOUND, '배틀 세션을 찾을 수 없습니다.');
  }
  ...
  // dungeonSession.notifyToOthers(accountId, payloadType, payload);
};
